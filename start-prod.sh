...
if docker compose -f docker-compose.local-prod.yml ps quiz_backend | grep -q "Up"; then
  ...
  docker compose -f docker-compose.local-prod.yml exec -T quiz_backend python manage.py download_official_icons --delay 1

  echo "  🔧 Сопоставление иконок с темами..."
  docker compose -f docker-compose.local-prod.yml exec -T quiz_backend python manage.py fix_icon_mapping
  ...
  echo "⚠️  Контейнер quiz_backend не запущен, пропускаем загрузку логотипов"
fi
fi
