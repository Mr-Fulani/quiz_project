    ...
    def send_to_telegram_action(self, request, queryset):
        ...
        for post in queryset:
            ...
            try:
                ...
                main_image = post.images.filter(is_main=True).first()
                main_media_added = False
                
                logger.info(f"Поиск медиа для поста '{post.title}'. Всего медиа: {post.images.count()}")
                ...
                if main_image:
                    ...
                    try:
                        ...
                        logger.warning(f"Не удалось получить главное медиа для поста '{post.title}': {e}")
                else:
                ...
                if not main_media_added:
                    logger.info(f"Поиск медиа по приоритету для поста '{post.title}'")
                    ...
                    if not photos_list and not gifs_list and not videos_list:
                        logger.warning(f"Не найдено медиа для поста '{post.title}' по приоритету")
