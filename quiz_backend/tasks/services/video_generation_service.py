...
def _generate_console_frame_vertical(
    ...
    # Логируем информацию о коде и пустых строках (только для первого кадра)
    if frame_index == 0:
        ...
        # Проверяем, сколько пустых строк в конце
        trailing_empty = 0
        for line in reversed(code_lines):
            if line.strip() == '':
                ...
                break
        logger.info(f"🔍 Пустых строк в конце кода: {trailing_empty}")
        if trailing_empty < 2:
            logger.warning(f"⚠️ В коде меньше двух пустых строк! Ожидалось 2, найдено {trailing_empty}")
