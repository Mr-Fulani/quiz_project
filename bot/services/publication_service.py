...
# заменена на расширенную publish_task_by_translation_group
...
# ) -> Tuple[bool, int, int, int]:
#     """
#     Публикует по одной самой старой задаче с переводами из каждого топика, где задачи
#     не публиковались более 30 дней или не публиковались вовсе. Игнорирует задачи без
#     переводов или с ошибками, продолжая обработку остальных топиков. Добавляет паузы
#     между публикациями для избежания блокировки Telegram.
#
#     :param translation_group_id: UUID группы переводов (игнорируется).
#     :param message: Объект сообщения для отправки ответов пользователю.
#     :param db_session: Асинхронная сессия базы данных.
#     :param bot: Экземпляр бота Aiogram.
#     :param admin_chat_id: ID чата администратора для отправки уведомлений.
...
#
#         # Определяем дату, старше которой задачи считаются "старыми" (30 дней назад)
#         utc_now = datetime.now(tz=pytz.utc)
#         one_month_ago = utc_now - timedelta(days=30)
#
#         # Получаем все топики с задачами, подходящими под условия
#         stmt = (
#             select(Topic)
...
#         default_link_service = DefaultLinkService(db_session)
#
#         # Собираем по одной самой старой задаче из каждого топика
#         tasks = []
#         for topic in topics:
#             stmt = (
#                 select(Task)
...
#                     (Task.publish_date.is_(None)) | (Task.publish_date < one_month_ago)
#                 )
#                 .order_by(Task.create_date.asc())  # Самая старая по дате создания
#                 .limit(1)
#             )
#             result = await db_session.execute(stmt)
#             task = result.unique().scalar_one_or_none()
...
#
#         for task in tasks:
#             # Проверяем наличие ошибок (хотя уже отфильтровано в запросе, для надёжности)
#             if task.error:
...
#                 continue
#
#             # Используем существующий URL изображения или генерируем новый
...
#                     group = task.group
#                     if not group:
#                         error_msg = f"🚫 Группа не найдена для языка {translation.language} в задаче {task.id}"
#                         logger.error(f"❌ {error_msg}")
...
#
#                     log_publication_success(task.id, translation.id, translation.language, target)
#                     await message.answer(f"✅ Публикована задача {task.id} ({translation.language}) в '{group.group_name}'.")
#
#                     # Пауза между публикациями (3-6 секунд)
#                     sleep_time = random.randint(3, 6)
#                     pause_msg = log_pause(sleep_time, task.id, translation.language, group.group_name)
#                     await message.answer(pause_msg)
#                     await asyncio.sleep(sleep_time)
#
#                 except Exception as e:
...
#                     task.error = True
#                     await db_session.commit()
#                     await message.answer(f"❌ Ошибка при публикации задачи {task.id} ({translation.language}): {e}")
#                     continue
#
#     except Exception as e:
...
#
#     # Итоговое обновление и сообщение
#     if published_count > 0:
#         try:
#             for task in tasks:
#                 if task.id in published_task_ids:
