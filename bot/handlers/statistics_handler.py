...
async def all_statistics(message: types.Message, db_session: AsyncSession):
    ...
    try:
        ...
        # Общая активность в каналах: прирост и убыль за последние 30 дней
        ...
        channel_activity = channel_activity_result.all()  # Список кортежей: (group_name, gained, lost)

        # Вычисляем общий прирост и убыль
        total_gained = sum(gained for _, gained, _ in channel_activity)
        total_lost = sum(lost for _, _, lost in channel_activity)

        if total_gained + total_lost > 0:
            overall_channel_activity = f"• *Активность в каналах*: \\+{total_gained} подписок / \\-{total_lost} отписок"
        else:
            overall_channel_activity = "• *Активность в каналах*: Нет изменений за последние 30 дней"

        # Детальная активность по каналам
        if channel_activity:
            channel_details = "\n• *Детальная активность по каналам*:\n"
            for group_name, gained, lost in channel_activity:
                channel_details += f"  \\- {escape_markdown(group_name)}: \\+{gained} / \\-{lost}\n"
        else:
            channel_details = "\n• *Детальная активность по каналам*: Нет данных за последние 30 дней"
